...
Для совместной работы над каким-то проектом, можно взять нужную (известную заранее) ссылку, либо найти через строку поиска интересующую нас тему.

Xтобы начать работать с чужим репозиторием, необходимо скопировать его в свой аккаунт через кнопку *Fork*. после этого нажимаем кнопку *Cjde* и копируем ссылку на этот репозиторий.

В *Visual Studio Code* создаем новую папку, открываем новый терминал и выполняем команду
~~~
git clone (Ссылка на скопированный репозиторий)
~~~
Чтобы посмотреть что содержится в этом репозитории нужно в него перейти

Для работы с чужим репозиторием (совместно работаем над каким-то проектом) нужно создать новую ветку, так как мы не можем направлять изменения в ветку *main*

После того как поработали с файлом и хотим его отправит на удаленный репозиторый, нужно выполнить команду
~~~
git push --set-upstream origin (название созданной ветка)

или

git push -u origin (название созданной ветка)
~~~

это создаст новую ветку на удаленном репозитории и передаст туда все данные с локального

Переходим в удаленный репозиторий, обновляем страницу, выбираем ветку которую создавали в локальном репозитории. Переходим на вкладку *Pull requests* далее нажимаем *New pull requests*, выбираем нужную ветку, далее жмем кнопку *Create pull requests*

Все, наши изменения и предложения ушли автору удаленного репозитория.